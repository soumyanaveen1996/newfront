(function() {
    ...
    var updateMessageUsage = function updateMessageUsage(botContext) {
        ...
        authContext
            ...
            .then(function(user) {
                ...
                var params = {
                    action: 'updateUsedGetAvailQuota',
                    stats: messageCount
                };
    ...
    var next = function next(message, state, previousMessages, botContext) {
        ...
        console.log('Got the message in background task bot!!!', msgType);
        switch (msgType) {
            ...
            case MessageTypeConstants.MESSAGE_TYPE_UPDATE_CALL_QUOTA:
                ...
                authContext
                    ...
                    .then(function(data) {
                        var _ = botContext.getCapability('Utils').Lodash;
                        var pstndata = _.get(data, '[0].pstn-minutes');
                        CallQuota({
                            error: null,
                            callQuota: pstndata.available || 0
                        });
                    })
                    .catch(function(err) {
