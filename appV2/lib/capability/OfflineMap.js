...
export default class OfflineMap {
    ...
    static saveMap(
        ...
        console.log('Mapbox start download', name, options);
        return Mapbox.offlineManager.createPack(
            options,
            (pack, status) => {
                console.log('Mapbox', pack, status);
                progressListener(pack, status);
            },
            (pack, err) => {
                console.log('Mapbox err', pack, err);
                errorListener(pack, err);
            }
        );
    }
    ...
    static deleteMap(name) {
        return Mapbox.offlineManager.deletePack(name);
    }
