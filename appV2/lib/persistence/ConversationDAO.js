                    ...
                    if (dbResults.length === 0) {
                        ...
                        let formattedResults = dbResults.map((dbResult) => {
                            return {
                                ...
                                unreadCount: dbResult.unreadCount
                            };
                        });
                ...
                function success(tx, res) {
                    ...
                    let dbConversations = res.rows
                        ? res.rows._array
                            ...
                            : []
                        : [];
                    let conversations = dbConversations.map(
                        (dbConversation) => {
                            return {
...
const addUnreadCount = () =>
    new Promise((resolve, reject) => {
        db.transaction((transaction) => {
            transaction.executeSql(
                conversationSql.addUnreadCount,
                [],
                () => resolve(true),
...
const resetUnreadCount = (conversationId) =>
    new Promise((resolve, reject) => {
        ...
        db.transaction((transaction) => {
            transaction.executeSql(
                conversationSql.resetUnreadCount,
                args,
                () => resolve(true),
                (tx, err) => reject(new Error('Unable to reset'))
            );
        });
    });
