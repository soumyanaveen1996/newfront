...
const createVersionTable = version =>
    new Promise((resolve, reject) => {
        db.transaction(tx => {
            tx.executeSql(
                dbVersionSql.createVersionTable,
                null,
                function success() {
...
const isVersionTablePresent = () =>
    new Promise((resolve, reject) => {
        db.transaction(tx => {
            tx.executeSql(
                dbVersionSql.tableExists,
                [],
                function success(tx2, res) {
                    res = Utils.addArrayToSqlResults(res);
                    let dbResults = res.rows
