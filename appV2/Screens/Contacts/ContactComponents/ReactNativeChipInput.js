...
const styles = (primaryColor, secondaryColor, variant) =>
    StyleSheet.create({
        inputAndChipWrapper: {
            // padding: 8,
            margin: 8
...
const ReactNativeChipInput = forwardRef((props: Props, ref) => {
    const {
        showChipIcon,
        variant,
        size,
    ...
    useImperativeHandle(ref, () => ({
        clearInputValues() {
            setInputValues([]);
            console.log('nput values are cleared.............');
        }
    }));
    ...
    const onDoneButtonPress = async () => {
        ...
        if (currentInputValue !== null && inputValues.length === 0) {
            ...
            onDone([...inputValues, currentInputValue]);
    ...
    const onRemoveValue = (value) => {
        ...
        onDone([...filterInputValues]);
    };

    const onBackKeyPress = (e) => {
        if (e.nativeEvent.key === 'Backspace' && currentInputValue === null) {
            const filterInputValues = inputValues.filter(
                (item) => item !== inputValues[inputValues.length - 1]
            );
            setInputValues([...filterInputValues]);
        }
    };
    ...
    return (
        <View
            style={[
                getInputVariantStyle(inputVariant),
                inputStyle ? inputStyle : themedStyles.inputAndChipWrapper
            ]}
        >
            ...
            <TextInput
                ...
                onKeyPress={
                    enableBackspaceDelete ? (e) => onBackKeyPress(e) : null
                }
                placeholderStyle={
                    placeholderStyle
                        ? placeholderStyle
                        : themedStyles.placeholderStyle
                }
