(function() {
  var ACCEPT_CONTACT_ACTION = 'AcceptContact';
  var IGNORE_CONTACT_ACTION = 'IgnoreContact';
  var SEND_IM_MSG_CAPABILITY = 'SendIMMessage';
  ...
  var decode = function decode(message, state, previousMessages, botContext) {
    ...
    return authContext
      ...
      .then(function(user) {
        if (message.getMessageType() === 'button_response') {
          var action = message.getMessage().action || '';
          var contact = message.getMessage().user;
          var Contact = botContext.getCapability('Contact');
          Contact.getContactFieldForUUIDs(contact.userId).then(function(
            contacts
          ) {
            if (_.isEmpty(contacts)) {
              var capParams = { users: [contact.userId] };
              ...
              if (ACCEPT_CONTACT_ACTION === action) {
                agentGuardService
                  ...
                  .then(function() {
                    ...
                    message.standardNotification(
                      'Contact ' + contact.userName + ' added'
                    );
                ...
                agentGuardService
                  ...
                  .then(function() {
                    ...
                    message.criticalNotification(
                      'Contact ' + contact.userName + ' ignored'
                    );
                    tell(message, botContext);
                  });
              }
            } else {
              var contactStatus = contacts[0].ignored ? 'ignored' : 'added';
              tell(
                'Contact ' + contact.userName + ' already ' + contactStatus,
                botContext
              );
            }
          });
  ...
  var sendImMessage = function sendImMessage(
    msg,
    botContext,
    user,
    previousMessages
    ...
    return conversationContext
      ...
      .then(function(conversation) {
        var isNewConversation = _.isUndefined(conversation.created);
        var agentGuardService = botContext.getCapability('agentGuardService');
        return agentGuardService.sendIMMessage(
          msg,
          isNewConversation,
          botContext,
          user,
  ...
  var asyncResult = function asyncResult(
    ...
    var _ = utils.Lodash;
    var content = _.get(result, 'details[0].message');
    ...
    if (content) {
      if (contentType == 30) {
      ...
      } else if (contentType == 270) {
        var msgInfo = _.get(result, 'details[0].info') || null;
        msgInfo = JSON.parse(msgInfo);
        switch (msgInfo.messageType) {
          case 'MISSED_CALL':
            var callTimestamp = _.get(msgInfo, 'callTimestamp');
            var date = callTimestamp ? new Date(callTimestamp) : new Date();
            message.standardNotification(
            ...
            break;
          default:
            message.standardNotification(content);
            break;
        }
      } else if (contentType == 290) {
        ...
        message.contactCard(content);
      } else if (
        MessageTypeConstantsToInt[
          MessageTypeConstants.MESSAGE_TYPE_OTHER_FILE
        ] == contentType
      ) {
        var options = _.get(result, 'details[0].options') || {};
        options = JSON.parse(options);
        message.otherFileMessage(content, options);
      } else if (contentType == 1000) {
        var contact = content;
        contact.waitingForConfirmation = false;
        var Contact = botContext.getCapability('Contact');
        return Contact.confirmContact(contact).then(function() {
          message.standardNotification(
            'You will be able to see ' + contact.userName + "'s details now"
          );
          tell(message, botContext);
        });
      } else {
    ...
    authContext
      ...
      .then(function(conversation) {
        var channelsInfo = conversation.onChannels;
        if (_.isEmpty(channelsInfo)) {
          var _Contact = botContext.getCapability('Contact');
          return _Contact.getContactFieldForUUIDs([result.createdBy]);
        } else {
          return ' ';
        }
      })
      ...
      .then(function(users) {
        if (
          !_.isEmpty(users) &&
          users.length > 0 &&
          !_.isEmpty(users[0].items) &&
          _.isUndefined(state['AddIgnoreMsgShown'])
        ) {
          var _contact = users[0].items[0];
          if (!_contact.visible) {
            delete _contact.phoneNumbers;
            delete _contact.emailAddress;
          }
          var msg = new Message();
          msg.buttonMessage(
            {
              title: _contact.userName,
  ...
  var getUserInfo = function getUserInfo(
    ...
    var params = {
      collection: 'People',
      query: [{ operand: 'userId', value: userId }],
      fields: ['emailAddress', 'userName', 'userId', 'visible', 'phoneNumbers'],
    };
    return agentGuardService.readData(
      null,
      botContext,
      user,
      previousMessages,
