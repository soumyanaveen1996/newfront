...
export default class ConversationList extends React.Component {
    ...
    async componentDidMount() {
        let conversations = await Promise.resolve(
            Conversation.getAllIMConversations()
        );
        conversations = conversations || [];

        let allChatsData = await Promise.all(
            _.map(conversations, async conversation => {
                let chatData = await Promise.resolve(
                    Utils.getMessageDataForConversation(conversation)
                );
                    ...
                    <ListView
                        ...
                        renderRow={conversation => (
                            <ConversationListItem
                                ...
                                chatData={conversation.chatData}
                            />
                        )}
