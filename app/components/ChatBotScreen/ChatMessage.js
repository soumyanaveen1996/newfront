...
export default class ChatMessage extends React.Component {
  ...
  renderMessage() {
    ...
    if (
      ...
      const component = (
        <View style={{ flexDirection: "column", width: "70%" }}>{buttons}</View>
      );
      return component;
    } else if (
      message.getMessageType() === MessageTypeConstants.MESSAGE_TYPE_FORM
  ...
  renderMetadata() {
    ...
    return (
      <View
        style={[metadataContainerStyle(alignRight, !!this.props.imageSource)]}
      >
        {message.getStatus() === 1 ? (
        ...
        >
          ...
          <View>
            <Text style={styles.sessionStartText}>
              {utils.sessionStartFormattedDate(message.getMessageDate())}
            </Text>
          </View>
          <View
            style={[
