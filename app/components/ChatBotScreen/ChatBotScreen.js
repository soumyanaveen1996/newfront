...
export default class ChatBotScreen extends React.Component {
    ...
    if (filteredMessages.length > 0) {
      for (var i = 0; i < filteredMessages.length; i++) {
        ...
        if (i < filteredMessages.length - 1) {
          ...
          if (
            !((nextDate.valueOf() - currentDate.valueOf()) / 1000 < 60) ||
  ...
  stopWaiting = () => {
    ...
    let messages = _.filter(
      this.state.messages,
      item =>
        item.message.getMessageType() !== MessageTypeConstants.MESSAGE_TYPE_WAIT
    );
  ...
  tell = message => {
    ...
    if (message.getMessageType() === MessageTypeConstants.MESSAGE_TYPE_SLIDER) {
      ...
      message.getMessageType() === MessageTypeConstants.MESSAGE_TYPE_BUTTON
    ) {
  ...
  replaceUpdatedMessage = updatedMessage => {
    const index = _.findIndex(
      this.state.messages,
      item => item.message.getMessageId() === updatedMessage.getMessageId()
    );
    if (index !== -1) {
  ...
  sendMessage = async message => {
    ...
    this.waitForQueueProcessing().then(() => {
      ...
      this.loadedBot
        .next(message, this.botState, this.state.messages, this.botContext)
        .then(response => {
  ...
  addSelectedContactsToBot = selectedRows => {
    if (selectedRows.length > 0) {
      try {
        ...
        ConversationContext.addParticipants(uuids, this.botContext).then(
          context => {
            this.conversationContext = context;
            let message = new Message();
      ...
      >
        <KeyboardAvoidingView
          ...
          keyboardVerticalOffset={
            Constants.DEFAULT_HEADER_HEIGHT + (Utils.isiPhoneX() ? 24 : 0)
          }
        >
          <FlatList
            accessibilityLabel="Messages List"
